...
export async function getAging(now = new Date()): Promise<AgingRow[]> {
  // Bolt optimization: Select only necessary fields to reduce DB transfer & memory overhead
...
export async function getSummary(now = new Date()): Promise<Summary> {
  // Bolt optimization: Select only required KPI fields, avoiding large text fields like notes/terms
...
export async function getMonthlyRevenue(monthsBack = 12, now = new Date()) {
  ...
  // Bolt optimization: Explicit select projections for monthly revenue reporting
...
export async function getTopClients(limit = 5) {
  // Bolt optimization: Projection to fetch only clientId, total amount, exchange rate, and client name
  const invoices = await prisma.invoice.findMany({
...
export async function getCurrencyExposure() {
  // Bolt optimization: Select minimal fields required for currency exposure calculation
  const invoices = await prisma.invoice.findMany({
    where: { status: { in: [...OPEN_FOR_AGG] } },
    select: {
      currency: true,
...
export async function getMethodBreakdown() {
  // Bolt optimization: Select minimal payment method and amount fields
