...
export async function getAging(now = new Date()): Promise<AgingRow[]> {
  // Optimization: fetch only required fields to avoid reading large text/json columns
...
export async function getSummary(now = new Date()): Promise<Summary> {
  // Optimization: select only fields required for aggregate calculations
...
export async function getMonthlyRevenue(monthsBack = 12, now = new Date()) {
  ...
  // Optimization: select only dates and amount fields needed for monthly aggregation
...
export async function getTopClients(limit = 5) {
  // Optimization: select only client ID/name and monetary fields
  const invoices = await prisma.invoice.findMany({
...
export async function getCurrencyExposure() {
  // Optimization: select only currency and balance component fields
  const invoices = await prisma.invoice.findMany({
    where: { status: { in: [...OPEN_FOR_AGG] } },
    select: {
      currency: true,
...
export async function getMethodBreakdown() {
  // Optimization: select only payment method, amount and locked FX rate
