...
export async function getAging(now = new Date()): Promise<AgingRow[]> {
  // Performance optimization: Select only necessary columns to avoid fetching unused fields/text blobs.
...
export async function getSummary(now = new Date()): Promise<Summary> {
  // Performance optimization: Select only status and monetary/date fields for dashboard KPI aggregation.
...
export async function getMonthlyRevenue(monthsBack = 12, now = new Date()) {
  ...
  const [invoices, payments] = await Promise.all([
    // Performance optimization: Select only required fields for monthly revenue calculation.
...
export async function getTopClients(limit = 5) {
  // Performance optimization: Select only client & total fields for client aggregation.
  const invoices = await prisma.invoice.findMany({
...
export async function getCurrencyExposure() {
  // Performance optimization: Select currency and settlement amounts.
  const invoices = await prisma.invoice.findMany({
    where: { status: { in: [...OPEN_FOR_AGG] } },
    select: {
      currency: true,
