...
export async function getAging(now = new Date()): Promise<AgingRow[]> {
  // Bolt optimization: select only required fields to minimize memory transfer and JSON serialization
...
export async function getSummary(now = new Date()): Promise<Summary> {
  // Bolt optimization: select only fields required for summary KPIs calculation
...
export async function getMonthlyRevenue(monthsBack = 12, now = new Date()) {
  ...
  // Bolt optimization: project only fields needed for monthly revenue aggregation
...
export async function getTopClients(limit = 5) {
  // Bolt optimization: select only clientId, client name, grandTotalCents, and exchangeRate
  const invoices = await prisma.invoice.findMany({
...
export async function getCurrencyExposure() {
  // Bolt optimization: select currency & balance components only
  const invoices = await prisma.invoice.findMany({
    where: { status: { in: [...OPEN_FOR_AGG] } },
    select: {
      currency: true,
...
export async function getMethodBreakdown() {
  // Bolt optimization: select payment method & amount plus invoice exchangeRate
