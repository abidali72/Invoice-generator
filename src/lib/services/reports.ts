...
export async function getAging(now = new Date()): Promise<AgingRow[]> {
  // Bolt optimization: select only required fields to avoid loading heavy text columns like notes/terms
...
export async function getSummary(now = new Date()): Promise<Summary> {
  // Bolt optimization: select only status/dates/monetary fields required for KPI calculations
...
export async function getMonthlyRevenue(monthsBack = 12, now = new Date()) {
  ...
  // Bolt optimization: limit Prisma query payloads to relevant columns for monthly revenue grouping
...
export async function getTopClients(limit = 5) {
  // Bolt optimization: fetch only client identifiers and totals to eliminate unnecessary model hydration
  const invoices = await prisma.invoice.findMany({
...
export async function getCurrencyExposure() {
  // Bolt optimization: project only currency and balance snapshot fields
  const invoices = await prisma.invoice.findMany({
    where: { status: { in: [...OPEN_FOR_AGG] } },
    select: {
      currency: true,
...
export async function getMethodBreakdown() {
  // Bolt optimization: project payment method and amount alongside locked exchange rate snapshot
