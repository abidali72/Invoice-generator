        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {fields.map((f) => (
            <Field key={f.key} label={`${f.label}${f.required ? " *" : ""}`} className={f.span === 2 ? "sm:col-span-2" : ""}>
              {f.kind === "textarea" ? (
                <textarea className="input" rows={3} value={(form[f.key] as string | number) ?? ""}
                  placeholder={f.placeholder}
                  onChange={(e) => setForm((s: Record<string, unknown>) => ({ ...s, [f.key]: e.target.value }))} />
              ) : f.kind === "select" ? (
                <select className="input" value={(form[f.key] as string | number) ?? ""}
                  onChange={(e) => setForm((s: Record<string, unknown>) => ({ ...s, [f.key]: e.target.value }))}>
                ...
                <input className="input" type={f.kind === "number" ? "number" : f.kind === "email" ? "email" : "text"}
                  ...
                  value={(form[f.key] as string | number) ?? ""} placeholder={f.placeholder}
                  onChange={(e) => setForm((s: Record<string, unknown>) => ({ ...s, [f.key]: e.target.value }))} />
              )}
            </Field>
          ))}
